...
1. **`move`** — map of `fromPath` → `toPath`. Rename an entity's slug (`"dark_haired_boy": "daniel"`) once its real name is learned, or relocate a facet. Never duplicate an entity under a new slug — rename it.
