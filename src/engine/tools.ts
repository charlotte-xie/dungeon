...
export const UPDATE_MEMORY_TOOL: ModelToolDefinition = {
    name: 'update_memory',
    description:
      `Update canonical long-term facts that should persist across scenes: recurring NPCs, locations, threads, and secrets. Apply the durability test — record only what will still be true and matter after the current scene ends (identity, appearance, motivations, relationships, secrets). Entries state present-tense truth, never running history: fold an event's consequences into the affected entries by rewriting them, and let the chronicle record the event itself. Never store temporary state: current positions, present company, momentary moods, and held or worn items are current-scene data, not memory. Call only for a material change. \`set\` maps lowercase underscore slugs to complete replacement descriptions; \`delete\` removes entries that are genuinely no longer relevant. Deletes apply before sets. Each description is limited to ${MAX_MEMORY_STRING_CHARS} characters.`,
    parameters: {
      type: 'object',
      properties: {
        set: {
